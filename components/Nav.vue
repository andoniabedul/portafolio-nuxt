<template>
  <nav class="nav">
    ...
    <div class="nav-drop">
      <button v-on:click="showMenu = !showMenu" class="nav-burger">
        <svg class="nav-burger-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <title>Menu</title>
    ...
    <transition name="slide-top">
      <div class="nav-menu" v-if="this.showMenu">
        <div class="nav-items">
          <!--
...
export default {
  data () {
    return {
      name: 'Andoni Abedul',
      showMenu: this.$device.isMobile ? false : true
    }
  },
  methods:{
    ...
    handleScroll (event) {
      ...
      var boundary = 50;
      if (window.scrollY > boundary && !this.hasClass('border-box', header)) {
        this.setClass('border-box', header);
        this.setClass('shadow-box', header);
      } else if (window.scrollY < boundary) {
        this.deleteClass('shadow-box', header);
        this.deleteClass('border-box', header);
      }
    }
  },
  created () {
...
<style lang="postcss">
  .nav {
    ...
    &-items {
      @apply w-full flex flex-col lg:flex-row text-sm justify-center lg:justify-end lg:content-end;
      .nav-icon {
